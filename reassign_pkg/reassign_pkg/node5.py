...
class TurtlebotController(Node):
    ...
    def move_callback(self):
        if self.should_move:
            ...
            lin_vel = min(lin_vel, self.max_vel)
    ...
    def can_reach(self, x, y, time_available):
        # Time = Distance/Speed
        error_x = x - self.pt_pose.x
        error_y = y - self.pt_pose.y
        dist= math.sqrt(error_x**2 + error_y**2)
        
        if abs(dist/self.max_vel) < time_available:
            return True
        return False
    ...
    def predict_pose(self, pt_list):
        ...
        # self.get_logger().info(f"(xc, yc, radius):{x_c},{y_c},{radius}")
        
        theta_1 = math.atan2(pt_list[1][0] - x_c, pt_list[1][1] - y_c)
        theta_2 = math.atan2(pt_list[2][0] - x_c, pt_list[2][1] - y_c)
        dtheta = theta_2-theta_1
        
        # predict velocity of RT and get half the velocity for PT  >> v=r*W
        self.max_vel = float(abs(dtheta*radius/10)) # /2*5sec
        ...
        for i in range(10):
            ...
            if self.can_reach(x_t, y_t, (i+1)*5):
                temp.x = float(x_t)
                temp.y = float(y_t)
                self.should_move = True
                self.get_logger().info(f"Target Locations : ({temp.x}, {temp.y}, [{second_pt+i}])")
                return temp
